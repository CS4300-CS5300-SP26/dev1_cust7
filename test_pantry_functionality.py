...
def test_pantry():
    ...
    response = session.post(f'{base_url}/pantry/add/', 
                          json=add_data,
    ...
    if response.status_code == 200:
        ...
        if len(recipes) > 0:
            ...
            print(f"   Best match: {recipes[0]['title']} ({recipes[0]['used_ingredient_count']} ingredients matched)")
        else:
    ...
    response = session.post(f'{base_url}/pantry/delete/{ingredient_id}/',
                          headers={'X-CSRFToken': session.cookies.get('csrftoken')})
    if response.status_code == 200:
        result = response.json()
        if result.get('success'):
