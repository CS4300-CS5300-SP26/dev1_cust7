...
class RecipeModelTests(TestCase):
    ...
    def test_recipe_ingredient_contains_name_quantity_and_unit(self):
        ...
        ingredient = RecipeIngredient.objects.create(
            recipe=recipe, name="Pasta", quantity="200", unit="g"
        )
    ...
    def test_recipe_ingredient_unit_is_optional(self):
        ...
        ingredient = RecipeIngredient.objects.create(
            recipe=recipe, name="Egg", quantity="2"
        )
    ...
    def test_recipe_ingredient_requires_quantity(self):
        ...
        ingredient = RecipeIngredient(
            recipe=recipe, name="Carrot", quantity="", unit="g"
        )
        with self.assertRaises(ValidationError):
            ingredient.full_clean()
    ...
    def test_recipe_ingredient_str(self):
        ...
        recipe = Recipe.objects.create(user=self.user, title="Waffles")
        ingredient = RecipeIngredient.objects.create(
            recipe=recipe, name="Flour", quantity="100", unit="g"
        )
    ...
    def test_recipe_can_be_set_to_public(self):
        ...
        recipe = Recipe.objects.create(
            user=self.user, title="Famous Cake", is_public=True
        )
...
class MissingCoverageTests(TestCase):
    ...
    def test_signin_invalid_credentials(self):
        ...
        response = self.client.post(
            reverse("signin"), {"username": "nonexistent", "password": "wrongpassword"}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_spoonacular_cache_timeout(self, mock_cache_set):
        ...
        with patch("home.spoonacular.cache.get", return_value=None):
            with patch("home.spoonacular.urllib.request.urlopen") as mock_urlopen:
                ...
                self.assertEqual(
                    call_args[1]["results"][0]["name"], "test"
                )  # Cached data
    ...
    def test_get_nutrition_missing_fields(self, mock_spoonacular):
        ...
        mock_spoonacular.side_effect = [
            {"results": [{"id": 1, "name": "banana"}]},  # Search succeeds
            {
                "id": 1,
                "name": "banana",
            },  # Nutrition lookup with minimal data (no nutrition key)
        ]
        response = self.client.get(reverse("get_nutrition", args=["banana"]))
    ...
    def test_edit_account_invalid_form(self):
        ...
        User.objects.create_user(
            username="otheruser", email="taken@email.com", password="pass123"
        )
    ...
    def test_signin_successful(self):
        ...
        response = self.client.post(
            reverse("signin"), {"username": "testuser", "password": "password123"}
        )
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse("index"))
    ...
    def test_create_recipe_empty_title(self):
        ...
        self.client.login(username="testuser", password="password123")
        response = self.client.post(
            reverse("create_recipe"), {"title": "", "is_public": "on"}  # Empty title
        )
...
class RecipeViewTests(TestCase):
    ...
    def test_recipe_view_ingredients_format(self):
        ...
        RecipeIngredient.objects.create(
            recipe=recipe, quantity="1", unit="cup", name="Flour"
        )
        RecipeIngredient.objects.create(
            recipe=recipe, quantity="2", unit="", name="Eggs"
        )
...
class SocialFeedTests(TestCase):
    ...
    def test_social_feed_shows_public_recipes(self):
        ...
        Recipe.objects.create(
            user=self.other_user, title="Public Pasta", is_public=True
        )
        response = self.client.get(reverse("social_feed"))
    ...
    def test_social_feed_does_not_show_private_recipes(self):
        ...
        Recipe.objects.create(
            user=self.other_user, title="Secret Soup", is_public=False
        )
        response = self.client.get(reverse("social_feed"))
    ...
    def test_social_feed_shows_recipes_from_all_users(self):
        ...
        Recipe.objects.create(
            user=self.other_user, title="Their Public Recipe", is_public=True
        )
        response = self.client.get(reverse("social_feed"))
    ...
    def test_social_feed_is_ordered_newest_first(self):
        ...
        older_recipe = Recipe.objects.create(
            user=self.user, title="Older Recipe", is_public=True
        )
        ...
        newer_recipe = Recipe.objects.create(
            user=self.other_user, title="Newer Recipe", is_public=True
        )
        ...
        for i in range(len(recipes) - 1):
            self.assertGreaterEqual(
                recipes[i].created_date, recipes[i + 1].created_date
            )
    ...
    def test_public_recipe_links_to_recipe_page(self):
        ...
        recipe = Recipe.objects.create(
            user=self.other_user, title="Linkable Recipe", is_public=True
        )
        response = self.client.get(reverse("social_feed"))
    ...
    def test_making_recipe_public_adds_it_to_feed(self):
        ...
        recipe = Recipe.objects.create(
            user=self.user, title="Soon Public", is_public=False
        )
        response = self.client.get(reverse("social_feed"))
    ...
    def test_making_recipe_private_removes_it_from_feed(self):
        ...
        recipe = Recipe.objects.create(
            user=self.user, title="Going Private", is_public=True
        )
        response = self.client.get(reverse("social_feed"))
    ...
    def test_social_feed_shows_recipe_author(self):
        ...
        Recipe.objects.create(
            user=self.other_user, title="Authored Recipe", is_public=True
        )
        response = self.client.get(reverse("social_feed"))
    ...
    def test_social_feed_shows_star_rating(self):
        ...
        recipe = Recipe.objects.create(
            user=self.other_user, title="Rated Recipe", is_public=True
        )
        RecipeRating.objects.create(recipe=recipe, user=self.user, stars=4)
        response = self.client.get(reverse("social_feed"))
...
class RegisterFormTests(TestCase):
    def test_register_form_rejects_duplicate_email(self):
        User.objects.create_user(
            username="existinguser", email="test@email.com", password="testpass123"
        )
...
class EditProfileFormTests(TestCase):
    ...
    def test_edit_profile_form_rejects_duplicate_email(self):
        User.objects.create_user(
            username="otheruser", email="taken@email.com", password="testpass123"
        )
        ...
        self.assertFalse(form.is_valid())
        self.assertIn("email", form.errors)
        self.assertIn("That email is already in use.", form.errors["email"])
