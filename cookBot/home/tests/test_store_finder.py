...
class KrogerStoreFinderTests(TestCase):
    ...
    def test_token_is_cached_between_requests(self, mock_urlopen):
        ...
        mock_urlopen.side_effect = [
            ...
            make_mock_response(
                MOCK_KROGER_STORE_RESPONSE
            ),  # second locations call (no token call)
        ]
    ...
    def test_recipe_page_serves_ingredients_as_objects_with_display_and_name(self):
        ...
        for ing in ingredients:
            self.assertIn(
                "display",
                ing,
                "Each ingredient must have a 'display' field for voice.js",
            )
            self.assertIn(
                "name",
                ing,
                "Each ingredient must have a 'name' field for pantry matching",
            )
    ...
    def test_recipe_page_pantry_names_are_lowercase(self):
        self.user.pantry_items.create(
            ingredient_name="Saffron"
        )  # stored with capital S
