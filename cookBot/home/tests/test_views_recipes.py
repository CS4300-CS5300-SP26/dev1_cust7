...
class CreateRecipeTests(TestCase):
    ...
    def test_empty_title_validation(self):
        ...
        response = self.client.post(reverse("create_recipe"), {"title": ""})
...
class EditRecipeTests(TestCase):
    ...
    def test_edit_recipe_updates_data(self):
        ...
        self.client.post(
            reverse("edit_recipe", args=[recipe.id]),
            {
                "title": "Updated",
                "is_public": "on",
...
class DeleteRecipeTests(TestCase):
    ...
    def test_delete_recipe(self):
        recipe = Recipe.objects.create(
            user=self.user, title="Delete Me", is_public=True
        )
...
class CreateRecipeImageAndDescriptionTests(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username="imguser", password="password123")
        self.client = Client()
...
class FavoriteRecipeTests(TestCase):
    ...
    def setUp(self):
        ...
        self.recipe = Recipe.objects.create(
            user=self.other, title="Favorite Me", is_public=True
        )
    ...
    def test_my_recipes_context_includes_favorite_ids(self):
        owned_recipe = Recipe.objects.create(
            user=self.user, title="Owned Recipe", is_public=True
        )
