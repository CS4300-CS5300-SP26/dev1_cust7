...
class SearchPageTests(TestCase):
    ...
    def setUp(self):
        ...
        self.vegan_tag, _ = Tag.objects.get_or_create(name='Vegan', defaults={'tag_type': 'dietary'})
        self.italian_tag, _ = Tag.objects.get_or_create(name='Italian', defaults={'tag_type': 'cuisine'})
        self.breakfast_tag, _ = Tag.objects.get_or_create(name='Breakfast', defaults={'tag_type': 'meal'})
    ...
    def test_filter_by_single_tag(self):
        response = self.client.get(reverse('search_recipes'), {'tags': self.vegan_tag.id})
        self.assertContains(response, 'Vegan Pasta')
        self.assertNotContains(response, 'Scrambled Eggs')
    ...
    def test_search_query_combined_with_tag_filter(self):
        response = self.client.get(
            reverse('search_recipes'),
            {'q': 'pasta', 'tags': self.vegan_tag.id}
        )
        self.assertContains(response, 'Vegan Pasta')
        self.assertNotContains(response, 'Scrambled Eggs')
