...
class AccountViewTests(TestCase):
    ...
    def test_signin_successful(self):
        ...
        response = self.client.post(
            reverse("signin"), {"username": "testuser", "password": "testpass123"}
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_register_form_invalid_data(self):
        ...
        response = self.client.post(
            reverse("register"),
            {"username": "test@user!", "password1": "123", "password2": "456"},
        )
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "form")
        form = response.context.get("form")
...
class IntegrationTest(TestCase):
    ...
    def test_full_user_journey(self):
        ...
        with self._mock_spoonacular():
            with patch(
                "home.views.generate_meal_plan_with_ai", return_value=MOCK_AI_MEALS
            ):
                ...
                self.assertTrue(
                    Pantry.objects.filter(
                        ingredient_name="Sugar", user=integration_user
                    ).exists()
                )
