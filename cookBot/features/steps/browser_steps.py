...
def step_add_recipe_steps(context):
    ...
    context.recipe_steps = [
        {"order": int(row["order"]), "text": row["text"]} for row in steps_table
    ]
    ...
    recipe = Recipe.objects.create(
        user=context.user, title=context.recipe_title, is_public=True
    )

    # Add ingredients
    for ing in context.recipe_ingredients:
        RecipeIngredient.objects.create(
            recipe=recipe, quantity=ing["quantity"], unit=ing["unit"], name=ing["name"]
        )

    # Add steps
    for step in context.recipe_steps:
        RecipeStep.objects.create(recipe=recipe, order=step["order"], text=step["text"])
...
def step_see_home_page(context):
    ...
    assert (
        ...
        or "home" in content.lower()
        or context.response.status_code == 200
    )
...
def step_see_meals_on_calendar(context, count):
    ...
    assert (
        meals.count() >= count
    ), f"Expected at least {count} meals, got {meals.count()}"
...
def step_meals_for_next_days(context, days):
    ...
    for meal in meals:
        ...
        assert (
            0 <= days_diff < days
        ), f"Meal date {meal.date} is not within next {days} days"
...
def step_redirected_to_home(context):
    ...
    if hasattr(response, "url"):
        ...
        assert any(
            url.endswith(reverse("index")) for url, _ in response.redirect_chain
...
def step_see_password_mismatch_error(context):
    ...
    assert context.response.status_code == 200, "Expected form to re-render with errors"
    assert (
        "password2" in content
    ...
    assert (
        "errorlist" in content or "error" in content.lower()
...
def step_see_username_exists_error(context):
    ...
    content = context.response.content.decode("utf-8")
    assert "username" in content.lower() and (
        "already exists" in content.lower() or "already taken" in content.lower()
...
def step_see_title_error(context):
    ...
    content = context.response.content.decode("utf-8")
    assert "title" in content.lower() and (
        "cannot be empty" in content.lower() or "required" in content.lower()
...
def step_see_email_in_use_error(context):
    ...
    assert "email" in content.lower() and (
        "already in use" in content.lower() or "already taken" in content.lower()
...
def step_see_duplicate_ingredient_error(context):
    ...
    content = context.response.content.decode("utf-8")
    assert (
        "already" in content.lower() or "duplicate" in content.lower()
...
def step_see_username_validation_error(context):
    ...
    assert (
        "username" in content.lower() or context.response.status_code == 200
...
def step_recipes_use_pantry_ingredients(context):
    ...
    pantry_ingredients = list(
        Pantry.objects.filter(user=context.user).values_list(
    ...
    for recipe in recipes:
        ...
        if any(
            ing.lower() in [p.lower() for p in pantry_ingredients]
