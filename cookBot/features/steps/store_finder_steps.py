...
def step_logged_in_viewing_recipe(context):
    ...
    RecipeIngredient.objects.create(
        recipe=recipe, name="saffron", quantity="1", unit="pinch"
    )
...
@given(
    "I am a logged in user with saffron in their pantry viewing a recipe with saffron"
)
...
def step_ingredients_have_correct_shape(context):
    ...
    assert (
        "ingredients-data" in content
    ), "ingredients-data script tag missing from page"
    import re
    import json
