...
def step_logged_in_viewing_recipe(context):
    ...
    context.user, created = User.objects.get_or_create(username="comment_user")
...
def step_submit_comment(context, comment_text):
    ...
    context.response = context.client.post(url, {"text": comment_text}, follow=True)
...
def step_not_logged_in_viewing_recipe(context):
    ...
    # Create or get test user
    other_user, created = User.objects.get_or_create(username="other_user")
    # Always hash password properly for both new and existing users
    ...
    # Create test recipe
    context.recipe, created = Recipe.objects.get_or_create(
        user=other_user, title="Public Recipe", defaults={"is_public": True}
    )
