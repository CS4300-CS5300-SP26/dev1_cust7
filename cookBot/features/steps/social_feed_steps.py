...
def step_create_public_recipe(context):
    context.recipe = Recipe.objects.create(
        user=context.user, title="My Public Pasta", is_public=True
    )
...
def step_create_private_recipe(context):
    context.recipe = Recipe.objects.create(
        user=context.user, title="Secret Stew", is_public=False
    )
...
def step_create_private_then_public(context):
    context.recipe = Recipe.objects.create(
        user=context.user, title="Soon Public", is_public=False
    )
...
def step_create_public_then_private(context):
    context.recipe = Recipe.objects.create(
        user=context.user, title="Going Private", is_public=True
    )
...
def step_two_users_share_recipes(context):
    ...
    context.recipe1 = Recipe.objects.create(
        user=context.user, title="My Public Recipe", is_public=True
    )
    context.recipe2 = Recipe.objects.create(
        user=other_user, title="Their Public Recipe", is_public=True
    )
    context.response = context.client.get(reverse("social_feed"))
...
def step_public_recipe_in_feed(context):
    context.recipe = Recipe.objects.create(
        user=context.user, title="Linkable Recipe", is_public=True
    )
    context.response = context.client.get(reverse("social_feed"))
